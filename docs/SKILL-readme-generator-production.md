---
name: readme-generator-production
description: Produce or rewrite an English README.md for a software repository as a reviewer-facing project entry point. Use when the task is to create, improve, audit, or standardize a README and the output must be grounded in repository evidence, optimized for first-pass clarity, and safe for production use by a coding LLM agent.
input-hint: Repository path or repository contents.
output-hint: README.md content or a README rewrite plan if key facts are missing.
---
